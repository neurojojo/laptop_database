...
% ===========================
% = Demo (stand-alone use): =
% ===========================
%
% wes_table = tracksTableClass('C:\teamtracking\WA_JM_reanalyze_CH01');
%
% * Note * the directory called should contain at least a Tracking.mat file 
%
% ===========================
% = from findFoldersClass   =
% ===========================
%
% Once findFoldersObj exists, 
% findFoldersObj.tracks.obj_# contains tracksTable and metadata
% 
% =============
% = Methods:  =
% =============
% [In-place] analyzeTracks: 
% [In-place] getTrackBoundaries: 
% [In-place] getBoundaryStats: 
% [In-place] getCellHeatmap:
% [In-place] getTrackLengths:
% [Overloaded] find:
% [Overloaded] hist:
% [Overloaded] plot:
% [Static] extractPosition:
% [Static] findFiles:
    ...
    properties
       ...
       tracksProximityTable % [Public] TABLE with the distance and time separating tracks
    end
    ...
    properties(GetAccess='private')
       ...
       distanceMatrix; % [Private] For use in analyzeTracks method
    end
...
%             
            ...
            % Check for an obj_idx integer and parse it if exists
            if nargin>1; obj_idx = varargin{2}; else obj_idx = 1; end
            ...
            % pipeline, look for them here - MESSY                      %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function analyzeTracks( obj, varargin )
            
            if ~isin( obj.tracksTable.Properties.VariableNames, 'trackLength' )
                obj.getTrackLengths;
            end
            ...
            if isempty(obj.distanceMatrix)
                obj.distanceMatrix = pdist2(  [x_end, y_end], [x_start, y_start ] ); % A distance matrix 
            end
            
            [particle1,particle2] = find( obj.distanceMatrix>0 & obj.distanceMatrix<10);
            
            particle_timeDiff = arrayfun( @(x) obj.tracksTable( particle1(x),: ).trackStart + ...
                obj.tracksTable(particle1(x),:).trackLength - obj.tracksTable(particle2(x),:).trackStart , [1:numel(particle1)] );
            
            mydistances = arrayfun(@(x) obj.distanceMatrix( particle1(x), particle2(x) ), [ 1:numel(particle1) ] );
            obj.tracksProximityTable = table( particle1, particle2, particle_timeDiff', mydistances',...
                'VariableNames', {'Track1','Track2','timeDiff','distanceDiff'} );
            
            obj.tracksProximityTable = obj.tracksProximityTable(abs(obj.tracksProximityTable.timeDiff)<100,:);
        end
...
% (4) Obtains hole information
            ...
            if gt(numel( obj.tracksTable ),0)
                ...
                obj.tracksTable.D_start_to_edge = min_values_starts';
                obj.tracksTable.D_end_to_edge = min_values_ends';
...
% Creates an image showing the locations where tracks are most likely to go
...
% Function which dissects tracksFinal into x,y (called from within the constructor function)
                ...
                % Fixed bug for compound tracks case
                start_time = arrayfun( @(x) x.seqOfEvents(1,1), tracksFinal )';
                end_time = arrayfun( @(x) x.seqOfEvents(end,1), tracksFinal )';
