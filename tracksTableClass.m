    ...
    methods
        ...
        function obj = tracksTableClass(varargin) 
            ...
            if ~any( cell2mat(regexp( fieldnames(fileStruct), 'address.*')) )
               error(sprintf('No addresses of files were found. We searched:\n%s\n...Did you enter the directory correctly?', Directory)); 
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%% TRACKING DATA PARSED HERE %%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            if ~fileStruct.Tracking & fileStruct.analysis_output; error( sprintf('No tracking file located even though the HMM analysis file exists at:\n%s', fileStruct.address_analysis_output) ); end
            if ~fileStruct.Tracking; error( sprintf('No tracking file located and no HMM analysis files located') ); end
            ...
            if isfield(fileStruct,'Tracking')
                try
                    ...
                    error('Tracking file does not contain tracksFinal variable'); 
                end
                    fileStruct.N_Tracking = max(size(tracksFinal.tracksFinal)); 
                    TrackingFile = fileStruct.address_Tracking; 
            end
            % TrackingPst file is located
            if isfield(fileStruct,'TrackingPst'); try; tmp=whos('-file',fileStruct.address_TrackingPst,'tracksFinal','size'); fileStruct.N_TrackingPst = max(tmp.size); catch; end; end;
            ...
            if isempty(TrackingFile); error('No suitable Tracking file located'); end
            % Locate the Tracking file in the Files cell
            
            % Using the tracksFinal variable, make a table
